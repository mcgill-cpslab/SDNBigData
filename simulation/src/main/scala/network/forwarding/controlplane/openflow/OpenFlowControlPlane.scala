...
class OpenFlowControlPlane (private [openflow] val node : Router)
  ...
  private def replyLLDP (pktoutMsg : OFPacketOut) {
    ...
    if (neighbor.nodeType != HostType &&
      neighbor.controlplane.isInstanceOf[OpenFlowControlPlane]) {
      neighbor.controlplane.asInstanceOf[OpenFlowControlPlane].
        sendLLDPtoController(outlink, lldpdata)
    }
  }
  ...
  private def getSwitchFeature (xid : Short) : OFFeaturesReply = {
    ...
    val featurelist = (node.DPID, 1000, flowtables.length, 0xff,
